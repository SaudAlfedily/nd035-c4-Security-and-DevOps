...
public class JWTAuthenticationVerificationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
        ...
        String heade = req.getHeader(SecurityConstants.HEADER_STRING);
        if (heade == null || !heade.startsWith(SecurityConstants.TOKEN_PREFIX)) {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest httpReq) {
        String token = httpReq.getHeader(SecurityConstants.HEADER_STRING);
        if (token != null) {
